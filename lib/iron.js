if(!window.iron) var iron = {};

iron = {
	markAsStatic : function(func){
		func.staticExtension = true;
		return func;
	},
	
	nativeExtend : function(NATIVE, extList){
		var StaticObject = {};
...
(function () {
    ...
    var primitiveJSON = function(key){
        return this.valueOf();
    };
...
iron.Class = function(InheritOpt, members){
	if(!members || !InheritOpt){
		var members = InheritOpt || {},
			Klass;

		Klass = iron.Class.create.call(iron.Class, members);
	}else{
		var SuperClass, MixTargets, Klass,
			isSingleton = false;
...
iron.Class.create = function(a, b){
	var args = Array.prototype.slice.apply(arguments),
		members = args.pop(),
		SuperClass = args.pop(),
		isSingleton = false;
...
iron.Object = iron.nativeExtend(Array, {
...
iron.Array = iron.nativeExtend(Array, {
	...
	forEach : function(callback, ctx){
		...
		while(i < ii){
			if(i in this){
				ctx ? callback.call(ctx, this[i], i, this) : callback(this[i], i, this);
			}
...
//		checkFunction(callback);
//
//		var hasInitVal = (arguments.length > 1),
//			ret = (hasInitVal) ? initVal : this[0],
//			i = (hasInitVal) ? 0 : 1,
//			isEmpty = true,
//			ii = this.length;
//
